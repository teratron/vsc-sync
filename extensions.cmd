...
REM Usage: extensions.cmd [command]
REM Commands:
...
if not exist "%EXTENSIONS_FILE%" (
    ...
    echo Use 'extensions.cmd list' first to create it.
    pause
    exit /b 1
)
...
echo.
echo Usage: extensions.cmd [command]
echo.
echo Commands:
...
echo.
echo Examples:
echo   extensions.cmd list      - Save current extensions
echo   extensions.cmd install   - Install from saved list
echo   extensions.cmd show      - View installed extensions
pause
exit /b 0
